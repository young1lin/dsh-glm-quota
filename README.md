...
- **渲染**：
  - 侧栏常驻部分固定为 **42px 状态胶囊**：`GLM · 套餐名` + 最高已用百分比 +
    微型圆环。它不再把完整额度卡长期塞在工作区列表下方，周额度存在时也不会继续
    挤压列表；点击后才在胶囊上方打开临时详情浮层，点外部或按 Esc 自动关闭
  ...
  - 侧栏 rail 态维持 36px SVG 圆环：进度 = 最高已用窗口，颜色取其档位；悬停
    Tooltip 显示完整摘要（含周额度百分比），点击刷新
