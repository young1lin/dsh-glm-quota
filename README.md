...
侧栏折叠 —— 纯额度竖排：5h 与周额度（存在时）各一枚小圆环，环下各自的重置倒计时：
...
- **渲染**：
  ...
  - 详情中每个窗口是一条紧凑的圆环指标：标签、精确百分比（MCP 为
    ...
    每 30s 更新。窗口按 API 返回**条件渲染**：**周额度（7d）只有存在时才出现** ——
  ...
  - 侧栏 rail 态为**纯额度竖排**：每个额度窗口（5h，及存在时的 7d）一枚 30px
    小圆环 —— 环弧即用量占比、环色取五档档位、环下是自身重置倒计时，环旁不
    渲染任何数字；不渲染 MCP 与套餐名，悬停 Tooltip 给出精确百分比与绝对重置
    时间，点击刷新。
