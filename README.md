...
侧栏折叠时逐窗口显示纯额度：5h（及存在时的周额度 7d）凹槽圆环，环弧示用量、环内显示重置倒计时，不含 MCP 次数与套餐名（精确百分比在悬停 Tooltip）。不足一分钟时倒计时显示秒数。
...
![侧栏展开：状态胶囊与详情浮层](assets/images/sidebar-expanded.png)

改版前侧栏折叠 —— 纯额度竖排：5h 与周额度（存在时）各一枚小圆环，环下各自的重置倒计时：

![侧栏折叠：纯额度圆环与重置倒计时](assets/images/sidebar-collapsed.png)
...
- **渲染**：
  - 侧栏常驻部分固定为 **42px 状态胶囊**：`GLM · 套餐名` + 汇总百分比 + 微型
    圆环。汇总只取**令牌额度窗口**（5h、7d 等）的最大值——语义是"离最近的令牌
    天花板还有多远"：MCP 是次数帽子不参与（MCP 95% + 5h 1% 时显示 1%），两个
    令牌窗口不一致时更接近上限的一方领先（7d 10% + 5h 5% → 10%）；仅剩 MCP
    数据时回退显示 MCP。它不再把完整额度卡长期塞在工作区列表下方，点击后才在
    胶囊上方打开临时详情浮层，点外部或按 Esc 自动关闭
  - 详情中每个窗口是一条紧凑的圆环指标：标签、精确百分比（MCP 为
    `已用 / 上限`）、自身的重置倒计时。绝对重置时间仍在原生 Tooltip 中；倒计时
    常态每 30s 更新，任一窗口进入最后一分钟后每秒更新。窗口按 API 返回**条件渲染**：**周额度（7d）只有存在时才出现** ——
    V1/Max 当前只显示 5 小时与 MCP，V2 及以上返回 7d 时自动多一行，无需改代码
  ...
  - 浮层头部保留手动刷新（请求在途时旋转）与陈旧数据提示；浮层使用不透明主题菜单
    表面、圆角与系统阴影，不遮透工作区文字
  - 与 DSH 原生拖拽联动：原生会话树是唯一滚动容器，拖拽进入 footer 胶囊时插件会
    继续推进该树的 `scrollTop`，因此拖动会话到侧栏底部不会被额度入口截断自动滚动
  - 收起侧栏为**纯额度竖排**：每个令牌额度窗口（5h，及存在时的 7d）一枚 46px
    凹槽圆环 —— 环弧即已用占比、环色取五档档位，圆环中心显示该窗口的重置倒计时；
    低于 1 分钟显示秒数。不渲染百分比、MCP 与套餐名，悬停 Tooltip 给出精确百分比
    与绝对重置时间，点击刷新。展开卡片的 28px 指标环和 22px 状态环沿用同一凹槽样式。
    周额度同样条件渲染（无 7d 窗口的套餐只显示 5h 一枚）；完全没有额度窗口时
    回退为旧的单圆环占位
